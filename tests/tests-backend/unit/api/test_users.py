...
async def override_get_current_user():
    return User(id=uuid4(), name="Test User", email="test@test.com", is_active=True, email_verified=False, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
...
async def test_create_user_api(async_client: AsyncClient):
    ...
    user_id = uuid4()
    mock_user = User(id=user_id, name="Test User", email="test@test.com", is_active=True, email_verified=False, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
