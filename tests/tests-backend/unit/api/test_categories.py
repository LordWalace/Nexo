...
async def test_create_category_api(async_client: AsyncClient):
    ...
    user_id = uuid4()
    mock_category = Category(id=uuid4(), name="Test Category", user_id=user_id, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
...
async def test_update_category_api(async_client: AsyncClient):
    ...
    cat_id = str(uuid4())
    user_id = uuid4()
    mock_category = Category(id=cat_id, name="Updated Category", user_id=user_id, created_at=datetime.now(UTC), updated_at=datetime.now(UTC))
    
    with patch("app.api.v1.categories.CategoryUseCases") as MockUseCases:
        mock_instance = MockUseCases.return_value
        mock_instance.update_category = AsyncMock(return_value=mock_category)
        
        response = await async_client.patch(f"/api/v1/categories/{cat_id}", json=payload)
