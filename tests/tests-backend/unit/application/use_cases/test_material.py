...
async def test_get_all_materials():
    ...
    expected_list = [
        Material(id=uuid4(), name="Mat1", url="http://test", user_id=user_id)
    ]
...
async def test_update_material_success():
    ...
    result = await use_cases.update_material(
        material_id=mat_id, user_id=user_id, material_in=update_data
    )
...
async def test_update_material_not_found():
    ...
    with pytest.raises(AppException) as excinfo:
        await use_cases.update_material(
            material_id=mat_id, user_id=user_id, material_in=update_data
        )
