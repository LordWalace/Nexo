...
async def test_update_history_success():
    ...
    existing_period = ActivityExecutionPeriod(
    ...
    result = await use_cases.update_history(
        period_id=period_id, user_id=user_id, history_in=update_data
    )
...
async def test_update_history_start_time():
    ...
    start_time = datetime.now(UTC)
    existing_period = ActivityExecutionPeriod(
        id=period_id, start_time=start_time, user_id=user_id, activity_id=uuid4()
    )
    mock_repo.get_by_id.return_value = existing_period
...
async def test_update_history_not_found():
    ...
    with pytest.raises(AppException) as excinfo:
        await use_cases.update_history(
            period_id=period_id, user_id=user_id, history_in=update_data
        )
