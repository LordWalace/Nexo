...
async def test_get_all_activities():
    ...
    expected_list = [Activity(id=uuid4(), title="Act1", user_id=user_id, category_id=uuid4())]
...
async def test_update_activity_success():
    ...
    existing_act = Activity(id=act_id, title="Act1", description="desc", category_id=cat_id, user_id=user_id)
    mock_repo.get_by_id.return_value = existing_act
    
    new_cat_id = uuid4()
    update_data = ActivityUpdate(title="ActUpdated", description="newdesc", category_id=new_cat_id)
    
    result = await use_cases.update_activity(activity_id=act_id, user_id=user_id, activity_in=update_data)
...
async def test_update_activity_not_found():
    ...
    with pytest.raises(AppException) as excinfo:
        await use_cases.update_activity(activity_id=act_id, user_id=user_id, activity_in=update_data)
...
async def test_delete_activity_success():
    ...
    existing_act = Activity(id=act_id, title="Act1", category_id=uuid4(), user_id=user_id)
    mock_repo.get_by_id.return_value = existing_act
